...
class ArchitexExtension {
    ...
    getArchitexPath() {
        // Try to find architex in PATH or use python -m architex
        const config = vscode.workspace.getConfiguration('architex');
        const customPath = config.get('architexPath');
        if (customPath) {
            return customPath;
        }
        // Default to python -m architex
        return 'python';
    }
    ...
    async exportDiagram() {
        ...
        const uri = await vscode.window.showSaveDialog({
            ...
            suggestedName: `architex_diagram.${format === 'mermaid' ? 'md' : format === 'plantuml' ? 'puml' : 'dot'}`
        });
        if (uri) {
            try {
                const workspacePath = vscode.workspace.workspaceFolders[0].uri.fsPath;
    ...
    async exportSummary() {
        ...
        const uri = await vscode.window.showSaveDialog({
            ...
            suggestedName: `architex_summary.${format === 'markdown' ? 'md' : 'json'}`
        });
        if (uri) {
            try {
                const workspacePath = vscode.workspace.workspaceFolders[0].uri.fsPath;
